"""
Sample Project V2 - Using New Requests API

This file demonstrates the expected result after API migration.
All timeout values are scaled by 1000 (seconds -> milliseconds)
All data parameters are renamed to json for consistency.
"""
...
def fetch_user_data(user_id: str) -> Dict[str, Any]:
    """Fetch user data using new API style."""
    url = f"https://api.example.com/users/{user_id}"
    
    # New API: timeout in milliseconds (scaled by 1000)
...
def create_user(user_data: Dict[str, Any]) -> Dict[str, Any]:
    """Create a new user using new API style."""
    url = "https://api.example.com/users"
    
    # New API: using 'json' parameter instead of 'data'
...
def update_user(user_id: str, updates: Dict[str, Any]) -> Dict[str, Any]:
    """Update user data using new API style."""
    url = f"https://api.example.com/users/{user_id}"
    
    # New API: PUT with json parameter, timeout in milliseconds
...
def delete_user(user_id: str) -> bool:
    """Delete a user using new API style."""
    url = f"https://api.example.com/users/{user_id}"
    
    # New API: DELETE with timeout in milliseconds
...
def batch_operations(operations: list) -> list:
    """Perform batch operations using new API style."""
    results = []
    
    for operation in operations:
        try:
            if operation['type'] == 'get':
                # New API: GET request with timeout in milliseconds
                ...
                # New API: POST with json parameter, timeout in milliseconds
                response = requests.post(
                    operation['url'], 
                    json=json.dumps(operation['payload']),
                    timeout=60*1000
                )
                results.append({
                    'id': operation['id'],
...
class APIClient:
    """API client using new requests API."""
    ...
    def get(self, endpoint: str, **kwargs) -> requests.Response:
        """GET request with timeout in milliseconds."""
    ...
    def put(self, endpoint: str, data: Any = None, **kwargs) -> requests.Response:
        """PUT request with json parameter."""
        url = f"{self.base_url}/{endpoint}"
